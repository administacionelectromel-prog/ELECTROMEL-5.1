...
export function buildCardWhatsApp(reg) {
  ...
  /* ── Segundo botón: mensaje con datos completos según tipo de doc ── */
  const tipo = (reg.tipo || (reg.numero || '').split('-')[0] || '').toUpperCase();
  const esIngreso = tipo.startsWith('ING');
  const esDoc = ['OTT', 'OTE', 'PRE', 'OT'].some(t => tipo.startsWith(t));
  if (esIngreso || esDoc) {
    const btnRico = document.createElement('button');
    btnRico.className = 'btn btn-ghost btn-block';
    btnRico.type = 'button';
    btnRico.style.marginTop = '8px';
    btnRico.textContent = esIngreso ? '📄 Enviar datos del ingreso' : '📄 Enviar presupuesto completo';
    btnRico.addEventListener('click', async () => {
      if (!reg.cliente_telefono) { import('../../core/ui.js').then(m => m.showToast('⚠️ Sin teléfono', 'warn')); return; }
      try {
        const varsMod = await import('../../services/whatsapp.vars.js');
        const plantilla = esIngreso ? 'ing_recibido' : 'doc_completo';
        const richVars = esIngreso ? await varsMod.varsIngreso(reg) : await varsMod.varsOrden(reg);
        const msg = await buildWhatsAppMessage(plantilla, richVars);
        openWhatsApp(reg.cliente_telefono, msg);
      } catch (e) {
        import('../../core/ui.js').then(m => m.showToast('No se pudo armar el mensaje', 'error'));
      }
    });
    card.appendChild(btnRico);
  }

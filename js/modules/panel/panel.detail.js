...
function _renderDetalle() {
  ...
  /* Acceso rápido a la OTT si el ingreso ya fue convertido.
     Sirve al escanear el QR de la etiqueta: lleva al ingreso, y desde
     acá se salta a la OT Taller para gestionar estado/pagos/entrega. */
  if (tipo === 'ING' && r.convertido_a_ott) {
    const cardOtt = document.createElement('div');
    cardOtt.className = 'card';
    cardOtt.style.borderColor = 'var(--acento)';
    cardOtt.innerHTML = `
      <div class="card-title">🔧 Orden de trabajo generada</div>
      <div class="dim txt-xs" style="margin-bottom:8px;">Este ingreso se convirtió en la OT Taller <b>${escapeHtml(r.convertido_a_ott)}</b>. Gestioná estado, pagos y entrega desde ahí.</div>
      <button class="btn btn-primary" type="button" style="width:100%;" onclick="window.abrirModalDetalle('${r.convertido_a_ott}','OTT')">🔧 Ir a ${escapeHtml(r.convertido_a_ott)} →</button>`;
    body.appendChild(cardOtt);
  }

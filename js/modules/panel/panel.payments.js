...
import { pesos, getTipoFromNumero, STORE_POR_TIPO, fechaHoy } from '../../core/utils.js';
...
export async function confirmarPagoParcial() {
  ...
  const fecha  = document.getElementById('pago-fecha')?.value || fechaHoy();
  ...
  try {
    ...
    /* Ofrecer enviar confirmación de pago por WhatsApp */
    if (orden.cliente_telefono) {
      const totalPagado = orden.r.pagos.reduce((a, p) => a + (parseFloat(p.monto) || 0), 0);
      const saldoRest = Math.max(0, (parseFloat(orden.total) || 0) - totalPagado);
      setTimeout(async () => {
        if (!(await confirmarLindo('¿Enviar confirmación de pago al cliente por WhatsApp?', { titulo: 'Confirmar pago', peligro: false, textoOk: 'Enviar' }))) return;
        try {
          const { buildWhatsAppMessage, openWhatsApp } = await import('../../services/whatsapp.js');
          const msg = await buildWhatsAppMessage('pago_confirmado', {
            cliente: orden.cliente_nombre,
            numero:  orden.numero,
            total:   monto,
            saldo:   saldoRest
          });
          openWhatsApp(orden.cliente_telefono, msg);
        } catch (e) { /* no crítico */ }
      }, 400);
    }

...
export async function abrirChecklist(clienteNombre, abono = null) {
  ...
  if (!maquinas.length) {
    showToast('Este cliente no tiene máquinas cargadas. Cargalas primero con 🏋️ en la suscripción.', 'warn');
    return;
  }
...
window._finalizarChecklist = async () => {
  try {
    ...
    /* Guardar en el historial de checklists del cliente */
    try {
      const { guardarChecklistHistorial } = await import('../services/flota.js');
      const hechas = detalle.filter(d => d.hecho).length;
      const obs = detalle.filter(d => d.estado === 'obs').length;
      await guardarChecklistHistorial(null, _ctx.clienteNombre, {
        total: detalle.length,
        hechas, observaciones: obs, bajas: bajas.length,
        detalle: detalle.map(d => ({ nombre: [d.marca, d.modelo].filter(Boolean).join(' '), numero: d.numero, estado: d.estado, hecho: d.hecho }))
      });
    } catch (e) { /* no crítico */ }

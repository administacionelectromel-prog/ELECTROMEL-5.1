...
import { pesos, fechaHoy, STORE_POR_TIPO, modalReady, mensajeAmigable, numSeguro } from '../core/utils.js';
...
window._recalcularSaldoOTT = function(origen) {
  const total = numSeguro(el('ott-total')?.value);
...
function _readOTT() {
  ...
  return {
    ...
    equipo_serie: store.get('ott.equipo_serie_heredada') || '',
...
export async function guardarOTT() {
  ...
  try {
    ...
    if (ESTADOS_ENTREGADO.includes(data.estado)) {
...
export async function crearOTTdesdeING(numIng) {
  ...
  modalReady(document.getElementById('modal-ott'), () => {
    ...
    /* Guardar la serie del ingreso para que viaje al OTT (sin campo visible) */
    store.set('ott.equipo_serie_heredada', ing.equipo_serie || '');

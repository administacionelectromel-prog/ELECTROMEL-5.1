...
export async function renderMantenimientos() {
  ...
  if (!lista.length) {
    cont.innerHTML = '<div class="dim txt-sm" style="padding:8px 0;">Sin mantenimientos programados.</div>';
    return;
  }
...
export async function agendarTurnoDesdeMant(id) {
  ...
  try {
    ...
    setTimeout(() => {
      const set = (idCampo, val) => { const e = document.getElementById(idCampo); if (e && val != null) e.value = val; };
      set('turno-cliente-nombre', m.cliente_nombre);
      set('turno-cliente-cuit',   m.cliente_cuit);
      set('turno-cliente-tel',    m.cliente_telefono);
      set('turno-cliente-dir',    m.cliente_direccion);
      set('turno-cliente-cp',     m.cliente_cp);
      set('turno-cliente-ciudad', m.cliente_ciudad || m.zona || '');
      set('turno-cliente-provincia', m.cliente_provincia);
      set('turno-servicio',       'Mantenimiento programado');
      set('turno-notas',          'Mantenimiento de: ' + (m.equipo || ''));

...
import { semanaRango, fechaHoy } from '../../core/utils.js';
...
export async function cargarTurnosSemana(rango) {
  ...
  /* Marcar qué turnos son de clientes CON abono (para no ofrecer generar
     OTE/PRE: esos ya tienen mantenimiento recurrente por su cuenta). */
  try {
    const abonos = await dbGetAll(db, 'abonos').catch(() => []);
    if (abonos && abonos.length) {
      const norm = (s) => (s || '').toString().trim().toLowerCase();
      const normTel = (s) => (s || '').toString().replace(/\D/g, '');
      const nombresAbono = new Set(abonos.map(a => norm(a.cliente_nombre)).filter(Boolean));
      const telsAbono = new Set(abonos.map(a => normTel(a.cliente_telefono)).filter(Boolean));
      for (const t of turnos) {
        const nom = norm(t.cliente_nombre);
        const tel = normTel(t.cliente_telefono);
        t._tieneAbono = (nom && nombresAbono.has(nom)) || (tel && telsAbono.has(tel));
      }
    }
  } catch (e) { /* sin abonos: ningún turno marcado */ }

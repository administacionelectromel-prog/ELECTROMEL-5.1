 ...
 */
export function scoreEvent(turno, costoViajeOverride = null) {
  if (!turno) return 50;
  ...
  /* Restar el costo de viaje a la ciudad del turno (si está configurada).
     ...
     Si nos pasan costoViajeOverride (costo repartido entre los trabajos del
     viaje activo), usamos ese en lugar del costo del viaje completo. */
  let costoViaje = 0, tiempoViaje = 0;
  if (costoViajeOverride !== null && costoViajeOverride !== undefined) {
    costoViaje = costoViajeOverride;
  } else {
    try {
...
export async function scoreEventAsync(turno) {
  ...
  /* ── Costo de viaje REPARTIDO: si hay un viaje activo a la ciudad del turno,
     el costo se reparte entre los trabajos del viaje (no se carga entero). ── */
  let costoViajeScore = null;
  try {
    const ciudad = turno.cliente_ciudad || turno.ciudad || '';
    if (ciudad) {
      const { costoViajeRepartido } = await import('../../services/gasto.operativo.js');
      const rep = await costoViajeRepartido(ciudad);
      if (rep && rep.hayViaje) costoViajeScore = rep.costoPorTrabajo;
    }
  } catch (e) { /* sin viaje activo: el score usa su lógica normal */ }
  ...
  try {
    ...
    if (stats.n >= 2) {
      ...
      /* Si hay viaje activo, sumar el costo de viaje repartido al costo del trabajo */
      const costoTotal  = avgCosto + (costoViajeScore || 0);
      const gananciaEst = ingreso - costoTotal;
      ...
      return { score, breakdown: { p1, p2, p3, fuente: 'historial', sample_n: stats.n, costoViajeRepartido: costoViajeScore } };
    }

    if (stats.n === 1) {
      return { score: scoreEvent(turno, costoViajeScore), breakdown: { fuente: 'cruzada', sample_n: 1 } };
    }
  } catch(e) { /* sin historial — caer a heurística */ }

  return { score: scoreEvent(turno, costoViajeScore), breakdown: { fuente: 'heuristica', costoViajeRepartido: costoViajeScore } };
}
...
export async function evaluarTurno(turno, eventosDelDia) {
  ...
  /* Costo de viaje repartido entre los trabajos del viaje activo (si lo hay) */
  let costoViajeScore = null;
  try {
    const ciudad = turno.cliente_ciudad || turno.ciudad || '';
    if (ciudad) {
  ...
  } catch (e) { /* sin viaje activo */ }

  const score     = scoreEvent(turno, costoViajeScore);

...
export async function renderAbonos() {
  ...
  if (!lista.length) {
    cont.innerHTML = q
      ...
      : '<div class="empty dim">Todavía no cargaste clientes con suscripción. Tocá el botón + para agregar.</div>';
    const resEl = document.getElementById('abonos-resumen');
  ...
  /* Resumen rápido arriba (como la franja de la Agenda) */
  const resumenEl = document.getElementById('abonos-resumen');
  if (resumenEl) {
    ...
    if (!total) {
      ...
      const partes = [];
      if (grupos.debe.length) {
        const deuda = grupos.debe.reduce((s, x) => s + (x.ec.deuda || 0), 0);
        partes.push(`<span class="rojo">${grupos.debe.length} debe${grupos.debe.length > 1 ? 'n' : ''} ${pesos(deuda)}</span>`);
      }
      if (grupos.porvencer.length) partes.push(`${grupos.porvencer.length} por vencer`);
      if (grupos.aldia.length) partes.push(`<span class="verde">${grupos.aldia.length} al día</span>`);
      resumenEl.innerHTML = `📋 <b>${total}</b> suscripci${total > 1 ? 'ones' : 'ón'} · ${partes.join(' · ')}`;
    }
  }
}
    ...
    <div class="abono-card" data-id="${a.id}">
      ...
      <div class="abono-detalle" id="abono-det-${a.id}">
        ...
        <div class="abono-acciones">
          ...
          <button class="btn btn-ghost btn-sm" type="button" onclick="_waAbono(${a.id})">💬 Recordar cobro</button>
...
window._borrarAbonoUI = async (id) => {
  if (!(await confirmarLindo('¿Eliminar este abono? No se puede deshacer.', { titulo: 'Eliminar' }))) return;
  try { await borrarAbono(id); showToast('Suscripción eliminada', 'success'); renderAbonos(); }
  catch (e) { showToast('❌ ' + mensajeAmigable(e), 'error'); }
};

/* ── Recordar cobro de abono por WhatsApp ──────────────── */
window._waAbono = async (id) => {
  try {
    const abonos = await listarAbonos();
    const a = abonos.find(x => x.id === id);
    if (!a) return;
    if (!a.cliente_telefono) { showToast('Este cliente no tiene teléfono cargado', 'warn'); return; }
    const { buildWhatsAppMessage, openWhatsApp } = await import('../services/whatsapp.js');
    const msg = await buildWhatsAppMessage('abono_cobro', {
      cliente: a.cliente_nombre,
      equipo:  a.equipo,
      total:   pesos(a.cuota)
    });
    openWhatsApp(a.cliente_telefono, msg);
  } catch (e) {
...
window._agendarVisitaAbono = async (id) => {
  try {
    ...
    setTimeout(async () => {
      ...
      precargarTurnoConCliente(a, {
        servicio: 'Mantenimiento programado',
        notas: 'Visita por abono: ' + (a.equipo || '')
      });
    }, 250);

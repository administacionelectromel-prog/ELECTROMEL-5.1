...
const BACKUP_STORES = [
  ...
  'abonos','mantenimientos',
  'viajes_operativos','ciudades_op','fotos'
];
...
export function importarBackup() {
  ...
  fileInput.onchange = async e => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (!(await confirmarLindo('⚠️ Importar este backup REEMPLAZARÁ todos los datos actuales. ¿Continuar?', { titulo: 'Restaurar backup' }))) return;
    try {
...
async function _restaurarBackup(dump) {
  ...
  setTimeout(async () => { if (await confirmarLindo('¿Recargar ahora?', { titulo: 'Recargar', peligro: false, textoOk: 'Recargar' })) window.location.reload(); }, 800);
}

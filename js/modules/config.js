...
import { showToast, actualizarBaseHeader, actualizarInfoSistema } from '../core/ui.js';
...
/* ═══════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════ */
const BACKUP_STORES = [
  'ingresos','ordenes','exteriors','presupuestos',
  'config','basePeriodos','finance_movements',
  'system_logs','clientes','rentabilidad_records','fallas'
];
...
export async function exportarBackup() {
  ...
  try {
    const { json, filename } = await _generarBackup(db);
    _descargarJSON(json, filename);
    await setCfg(db, 'last_backup_ts', new Date().toISOString());
    _actualizarLabelBackup();
    await logEvent(db, { type: 'BACKUP_CREATED', message: 'Backup: ' + filename });
...
async function _generarBackup(db, filenameOverride) {
  const dump = {
    meta: { app: 'ELECTROMEL', version: DB_VERSION, exported_at: new Date().toISOString() },
    data: {}
  };
  await Promise.all(BACKUP_STORES.map(async s => {
    try { dump.data[s] = await dbGetAll(db, s, false); } catch(e) { dump.data[s] = []; }
  }));
  const json     = JSON.stringify(dump, null, 2);
  const _ahora   = new Date();
  const _fecha   = _ahora.toISOString().slice(0, 10);
  const _hora    = _ahora.toTimeString().slice(0, 5).replace(':', '');
  const filename = filenameOverride || ('electromel_backup_' + _fecha + '_' + _hora + '.json');
  return { json, filename, dump };
}

function _descargarJSON(json, filename) {
...
export function importarBackup() {
  ...
  fileInput.onchange = async e => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (!confirm('⚠️ Importar este backup REEMPLAZARÁ todos los datos actuales. ¿Continuar?')) return;
    try {
      const text = await file.text();
      const dump = JSON.parse(text);
      if (!dump?.data) throw new Error('Formato de backup inválido');
      await _restaurarBackup(dump);
    } catch(err) {
...
async function _restaurarBackup(dump) {
  const db = store.get('db');
  showToast('Restaurando backup...', 'info');
  const stores = Object.keys(dump.data).filter(s => BACKUP_STORES.includes(s));
  ...
  for (const storeName of stores) {
    await new Promise((resolve, reject) => {
      try {
        ...
        cr.onsuccess = () => {
          const records = dump.data[storeName] || [];
  ...
  invalidateCache();
  await logEvent(db, { type: 'BACKUP_RESTORED', message: 'Backup importado', data: { stores } });
  showToast('✅ Backup restaurado — recargá la página', 'success');
  setTimeout(() => { if (confirm('¿Recargar ahora?')) window.location.reload(); }, 800);
}
...
function _actualizarLabelBackup() {
  ...
  getCfg(db, 'last_backup_ts', null).then(ts => {
    ...
    if (!el) return;
    if (!ts) { el.textContent = 'Sin backups previos.'; return; }
    const days = Math.floor((Date.now() - new Date(ts).getTime()) / 86400000);
    const fecha = new Date(ts).toLocaleDateString('es-AR');
    el.textContent = days === 0 ? 'Último backup: hoy' : days === 1 ? 'Último backup: ayer' : `Último backup: ${fecha} (hace ${days} días)`;
  }).catch(() => {});
}
...
export async function ejecutarResetCompleto() {
  ...
  try {
    ...
    const backup = await _generarBackup(db, fname);
    _descargarJSON(backup.json, backup.filename);
    await setCfg(db, 'last_backup_ts', new Date().toISOString());
...
function _buildConfigPanel() {
  ...
  cont.innerHTML = `
    ...
    <!-- ── BASE PERÍODOS ───────────────────────────────── -->
    <div class="cfg-section" data-cfg-title="✈️ Viajes de trabajo por zona">
      <div class="dim txt-sm mb-6">Tu base es San Martín de los Andes. Acá cargás los viajes a otras zonas: fechas, y los gastos del viaje (costo por día y pasaje) que se reparten entre los trabajos de ese viaje.</div>
      <datalist id="bp-zonas-datalist"></datalist>
      <div id="cfg-baseperiodos-list" class="bp-list"></div>
      <button class="btn btn-ghost btn-block btn-sm" type="button" onclick="agregarBasePeriodo()">+ Agregar viaje</button>
    </div>

    <!-- ── BASES Y ZONAS ───────────────────────────────── -->
    <div class="cfg-section" data-cfg-title="📍 Bases y zonas de influencia">
      <div class="dim txt-sm mb-6">Dirección de cada base y las ciudades de su zona, con el costo de viaje a cada una. El motor de agenda usa esto para calcular la conveniencia real de cada trabajo.</div>

      <div class="card">
        <div class="card-title">🏠 SMA — San Martín de los Andes</div>
        ...
        <div class="field" style="max-width:140px;"><label class="field-label">Radio (km)</label><input type="number" id="cfg-base-sma-radio" min="0" step="10" placeholder="200" inputmode="numeric"></div>
      </div>

      <div class="card">
        <div class="card-title">🚐 NQN — Neuquén</div>
        <div class="field"><label class="field-label">Dirección de referencia</label><input type="text" id="cfg-base-nqn-dir" placeholder="Calle y número"></div>
        <div class="field" style="max-width:140px;"><label class="field-label">Radio (km)</label><input type="number" id="cfg-base-nqn-radio" min="0" step="10" placeholder="200" inputmode="numeric"></div>
      </div>

      <button class="btn btn-ghost btn-block btn-sm mb-6" type="button" onclick="guardarBasesInfo()">💾 Guardar direcciones</button>

      <div class="card-title">🏙️ Ciudades y costo de viaje</div>
      <div id="cfg-ciudades-list" class="bp-list"></div>
      <button class="btn btn-ghost btn-block btn-sm" type="button" onclick="abrirFormularioCiudad()">+ Agregar ciudad</button>
    </div>
    ...
    <!-- ── INTELIGENTE ─────────────────────────────────── -->
    <div class="cfg-section" data-cfg-title="🧠 Motor inteligente">
      <div class="field-row">
        <div class="field"><label class="field-label">Ganancia mínima por trabajo</label><input type="number" id="cfg-ganancia-min-trabajo" min="0" placeholder="15000"></div>
        <div class="field"><label class="field-label">Score mínimo para aceptar</label><input type="number" id="cfg-score-acepta-min" min="0" max="100" placeholder="60"></div>
      </div>
      <div class="field-row">
        <div class="field"><label class="field-label">Horas/día máximas</label><input type="number" id="cfg-horas-dia-max" min="1" max="24" placeholder="8"></div>
        <div class="field"><label class="field-label">Km a NQN</label><input type="number" id="cfg-km-nqn" min="0" placeholder="120"></div>
      </div>
      <div class="field"><label class="field-label">Costo por km</label><input type="number" id="cfg-costo-km" min="0" placeholder="350"></div>
    </div>
    ...
    <div class="cfg-section" data-cfg-title="ℹ️ Información del sistema">
      ...
      <div class="dim txt-sm" style="margin-top:4px;">App versión: <span id="info-app-version">6.6</span></div>
...
function _buildCollapsibles() {
  ...
  panel.querySelectorAll('.cfg-section').forEach((sec, idx) => {
    ...
    wrap.className = 'collapsible' + (idx === 0 ? ' open' : '');

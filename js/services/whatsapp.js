...
export function buildWhatsAppMessage(estadoOTemplate, vars = {}) {
  function _interpolar(tmpl) {
    ...
    return String(tmpl)
      ...
      .replace(/\{diagnostico\}/g, vars.diagnostico || '')
      .replace(/\{trabajo\}/g,   vars.trabajo   || '')
      .replace(/\{adelanto\}/g,  vars.adelanto  || '')
      .replace(/\{contra_entrega\}/g, vars.contra_entrega || '')
      .replace(/\{dias_entrega\}/g, vars.dias_entrega || '')
      .replace(/\{banco\}/g,     vars.banco     || '')
...
/* ── GANCHO PARA APP NATIVA: enviar PDF adjunto por WhatsApp ──
   Hoy (PWA) WhatsApp no permite adjuntar archivos desde la web,
   así que esta función solo abre el chat con el texto.
   En la app nativa, acá se conectará el envío del PDF adjunto
   (vía share nativo / WhatsApp Business API).
   `generarPdfFn` es opcional: una función que genera/devuelve el PDF. */
export async function enviarWhatsAppConPDF(telefono, mensaje, generarPdfFn) {
  // NATIVA (futuro): si hay capacidad de compartir archivos, usarla.
  // if (window.NativeShare?.canShareFiles && generarPdfFn) {
  //   const blob = await generarPdfFn();
  //   return window.NativeShare.shareFile(blob, telefono, mensaje);
  // }
  // PWA (hoy): abrir el chat con el texto. El usuario adjunta el PDF a mano si quiere.
  openWhatsApp(telefono, mensaje);
}

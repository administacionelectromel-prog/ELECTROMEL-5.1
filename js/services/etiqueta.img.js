...
/* ── Descargar una imagen (dataURL) como archivo ────────────
   Usa Blob para que sea robusto en Android/Chrome (igual que el
   doc.save de los PDF). */
function _descargarImagen(dataUrl, filename) {
  try {
...
export async function etiquetaImagenING(numero) {
  ...
  try {
    ...
    _descargarImagen(cvs.toDataURL('image/jpeg', 0.95), numero + '_etiqueta.jpg');
    await logEvent(db, { type: 'IMG_GENERATED', message: 'ING etiqueta img: ' + numero, ref: numero }).catch(()=>{});
    showToast('🏷️ Etiqueta descargada (JPG)', 'success');
  } catch (err) {

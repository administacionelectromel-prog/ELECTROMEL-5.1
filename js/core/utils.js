...
export function fechaHoy() {
  /* Fecha local del dispositivo (NO UTC). Usar toISOString() daba el día
     equivocado entre las ~21:00 y medianoche en Argentina (UTC-3),
     porque toISOString() devuelve la hora UTC. */
  const d = new Date();
...
export function buildWaPhone(tel) {
  ...
  /* Formato local argentino:
     - Quitar el 0 inicial de código de área (ej: 02972...)
     - Quitar el 15 que va después del área en celulares viejos
       (ej: 2972 15 456789 → 2972 456789) */
  if (t.startsWith('0')) t = t.slice(1);          // saca 0 de área

  /* Detectar y quitar el "15" del celular. El formato es:
     [área 2-4 díg][15][número 6-8 díg]. Si después del área hay un 15,
     lo removemos. Heurística: si el número tiene un 15 entre el área y
     el resto y la longitud total lo sugiere. */
  const m = t.match(/^(\d{2,4})15(\d{6,8})$/);
  if (m) t = m[1] + m[2];                          // saca el 15 intermedio
...
/* ── numSeguro(valor, porDefecto=0) ─────────────────────────
   Convierte cualquier valor a número de forma segura.
   Si el resultado es NaN, Infinity o inválido, devuelve el
   valor por defecto. Evita guardar totales/pagos corruptos. */
export function numSeguro(valor, porDefecto = 0) {

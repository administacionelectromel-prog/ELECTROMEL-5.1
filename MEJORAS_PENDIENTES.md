...
8. **OCR de facturas (para la app nativa) — estructura a dejar preparada:**
   Cargar un egreso sacando foto de la factura, que el OCR la lea y complete el formulario solo.
   - **Ahora NO se hace** (la app es PWA offline; el OCR bueno con IA necesita internet y tiene costo; ML Kit es solo nativo). Se deja para la app nativa.
   - **Cuando se haga, dejar la estructura para que el OCR se enchufe sin rehacer nada:**
     - Campos en el egreso: `imagen_factura`, `proveedor`, `numero_factura`, `ocr_procesado` (flag), `ocr_texto_crudo`.
     - Botón "📷 Cargar factura" + lugar para los datos auto-completados (editables a mano).
     - Función de enganche `procesarFacturaOCR(imagen)` que la app nativa completará con ML Kit / IA.
   - **Enfoque por pasos**: (1) foto adjunta al egreso, offline, simple; (2) OCR local (Tesseract); (3) OCR + IA si la precisión lo justifica.
   - El flujo ideal: 📷 Cargar factura → foto → OCR → proveedor/fecha/importe/categoría auto → Guardar en Egresos, guardando la imagen original.
